...
| 分支 | 用途 | 目录边界 |
| --- | --- | --- |
| `main` | 完整集成分支，包含 Tk 与 Vue 两个版本的可公开内容 | `README.md`、`AGENTS.md`、`.gitignore`、`docs/product/`、`src-prototype/`、`src-tauri-vue/`、经确认可公开的 `assets/` |
...
```powershell
cd E:\ProgrammingAlgorithm\VSCodeProjects\DanhuangProduct-tk
git init -b main
...
后续所有提交都要显式标明线别。Tk/Python 与 Vue/Tauri 是平级版本，不用其中一个覆盖另一个。`main` 作为完整集成分支，最终包含两个版本的可公开源码和文档；`tk/main`、`vue/main` 用于各自版本的日常推进。更细规则见：[蛋黄双版本分支与提交规范 v1](蛋黄-双版本分支与提交规范-v1.md)。
...
Tk 改动必须同步中文文档，文档标题或正文标明 `目标线别：Tk/Python 线`。例如：
...
Vue/Tauri 改动必须同步中文文档，文档标题或正文标明 `目标线别：Vue/Tauri 线`。Tk 和 Vue 各自验证后，再合入 `main`，保证主分支包含全部可公开提交内容。

合并时通过 GitHub Pull Request 先合到对应主线：Tk 功能合到 `tk/main`，Vue 功能合到 `vue/main`。随后把已验证的 `tk/main` 和 `vue/main` 合到 `main`，让 `main` 保持完整集成视图。
