...
已基于 Tk 版本 `0.11.65` 在当前根 `E:\ProgrammingAlgorithm\VSCodeProjects\DanhuangProduct-tk` 生成新的 Windows 免 Python exe 包。包产物仍留在本机 `packages/`，不进入 GitHub 代码仓库；GitHub 只提交本记录和源码/文档变更。

按照用户最新要求，`packages/` 每次打包后只保留最新一组 Tk Windows 包目录和 zip。本批新包验证通过后，已删除旧 0.11.64 包目录和 zip，当前只保留 0.11.65。

本批发行包按“蛋黄单宠版”处理：`pet-family.json` 只包含 `danhuang`，`identity_image=""`，`reference_images=[]`，不带用户上传源图、现实照片、身份参考图、个人故事或跨宠物扩展动作。

0.11.65 的新增重点是安全页公开分发体验可信度：导出安装包弹窗的导出完成、导出失败、缺少 GitHub 仓库、缺少 GitHub Token、macOS 远端构建完成/失败统一使用状态卡和面板 Toast，不再弹出 Windows 系统信息框/错误框。

## 包路径

```text
E:\ProgrammingAlgorithm\VSCodeProjects\DanhuangProduct-tk\packages\danhuang-desktop-pet-windows-20260706-210701
E:\ProgrammingAlgorithm\VSCodeProjects\DanhuangProduct-tk\packages\danhuang-desktop-pet-windows-20260706-210701.zip
```

## 哈希

```text
ZIP SHA256: 24F268CC65760B2F8EDBD142FA0067D938169495F9CADE13A76D7FEFDF5B9FEA
...
- `python -m unittest discover -s src-prototype/modular/tests` 通过 42 个测试。
...
- 发行包 `validate_phase3.py --pet-dir packages/danhuang-desktop-pet-windows-20260706-210701/app --json` 输出 1 个 ready 宠物、19 个蛋黄动作、0 个扩展动作，右键菜单布局 `columns=3/min_width=540/max_width=620/max_height_ratio=0.78`，分组顺序为 `common/window/activity_modes/base_actions/extension_actions`，`warnings` 为空。
- 包内 29 个 JSON 可解析；源码、文档和本批发行包内文本文件未检测到 UTF-8 BOM 或非法 UTF-8。
- 包内 `manifest.json` 显示 `app_version=0.11.65`、`python_bundled=true`、`windows_exe=app/dist/DanhuangDesktopPet/DanhuangDesktopPet.exe`、`missing_assets=[]`。
...
- exe 8 秒桌面启动烟测通过，截图见 `qa/tk-ui-0.11.65-installer-export-feedback-20260706/exe-smoke-desktop.png`；结束后无本次启动的 `DanhuangDesktopPet` 残留进程。
...
- `packages/` 当前只保留最新 0.11.65 目录和 zip：`packages/danhuang-desktop-pet-windows-20260706-210701/`、`packages/danhuang-desktop-pet-windows-20260706-210701.zip`。旧包已按用户最新要求清理，不再累积。

## 回退方式

- 本地只保留最新包；如需回退上一版，需要从 Git 历史恢复 0.11.64 源码后重新打包，或从外部备份取回旧 zip。
- 代码回退：回退 `APP_VERSION=0.11.64`，恢复导出安装包弹窗的上一版反馈实现并移除对应测试；注意这会恢复导出完成/失败和 GitHub 前置错误时弹出 Windows 系统框的付费用户体验风险。
