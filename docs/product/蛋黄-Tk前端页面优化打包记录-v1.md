...
已基于 Tk 版本 `0.11.68` 在当前根 `E:\ProgrammingAlgorithm\VSCodeProjects\DanhuangProduct-tk` 生成新的 Windows 免 Python exe 包。包产物仍留在本机 `packages/`，不进入 GitHub 代码仓库；GitHub 只提交本记录和源码/文档变更。

按照用户最新要求，`packages/` 每次打包后只保留最新一组 Tk Windows 包目录和 zip。本批新包验证通过后，已删除旧 0.11.67 包目录和 zip，当前只保留 0.11.68。

本批发行包按“蛋黄单宠版”处理：`pet-family.json` 只包含 `danhuang`，`identity_image=""`，`reference_images=[]`，不带用户上传源图、现实照片、身份参考图、个人故事或跨宠物扩展动作。

0.11.68 的新增重点是聊天可信度：聊天窗口重复打开时恢复并聚焦输入框；聊天背景上传失败改为当前窗口 Toast；背景窗口补 `Esc` 关闭和焦点提升。

## 包路径

```text
E:\ProgrammingAlgorithm\VSCodeProjects\DanhuangProduct-tk\packages\danhuang-desktop-pet-windows-20260706-214445
E:\ProgrammingAlgorithm\VSCodeProjects\DanhuangProduct-tk\packages\danhuang-desktop-pet-windows-20260706-214445.zip
```

## 哈希

```text
ZIP SHA256: DD419374923D44476FA038F6D93CFB1F5B87692E978DDB843A6D0B82948959F3
...
- `python -m unittest discover -s src-prototype/modular/tests` 通过 46 个测试。
...
- 发行包 `validate_phase3.py --pet-dir packages/danhuang-desktop-pet-windows-20260706-214445/app --json` 输出 1 个 ready 宠物、19 个蛋黄动作、0 个扩展动作，右键菜单布局 `columns=3/min_width=540/max_width=620/max_height_ratio=0.78`，分组顺序为 `common/window/activity_modes/base_actions/extension_actions`，`warnings` 为空。
- 包内 29 个 JSON 可解析；源码、文档和本批发行包内文本文件未检测到 UTF-8 BOM 或非法 UTF-8。
- 包内 `manifest.json` 显示 `app_version=0.11.68`、`python_bundled=true`、`windows_exe=app/dist/DanhuangDesktopPet/DanhuangDesktopPet.exe`、`missing_assets=[]`。
...
- exe 8 秒桌面启动烟测通过，截图见 `qa/tk-ui-0.11.68-chat-focus-background-20260706/exe-smoke-desktop.png`；结束后无本次启动的 `DanhuangDesktopPet` 残留进程。
...
- `packages/` 当前只保留最新 0.11.68 目录和 zip：`packages/danhuang-desktop-pet-windows-20260706-214445/`、`packages/danhuang-desktop-pet-windows-20260706-214445.zip`。旧包已按用户最新要求清理，不再累积。

## 回退方式

- 本地只保留最新包；如需回退上一版，需要从 Git 历史恢复 0.11.67 源码后重新打包，或从外部备份取回旧 zip。
- 代码回退：回退 `APP_VERSION=0.11.67`，移除聊天窗口重复打开聚焦、聊天背景失败 Toast、背景窗口 `Esc` 关闭和对应测试；注意这会恢复聊天入口弱焦点和系统弹窗割裂体验。
